...
-- ⑤ job_post 생성
-- JobSourceType enum: ADMIN / USER / SCRAPED  ('MANUAL' 없음 → ADMIN으로 교체)
-- RegistrationStatus enum: DRAFT / CONFIRMED / CANCELED  ('REGISTERED' 없음 → CONFIRMED으로 교체)
...
-- ⑥ 지원 이력
-- ApplicationStatus enum: SUBMITTED / ACTIVE / ARCHIVED
-- 'APPLIED'는 enum에 없으므로 서류 제출 완료 상태인 SUBMITTED으로 교체
