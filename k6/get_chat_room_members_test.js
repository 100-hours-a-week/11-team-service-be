...
const duration = new Trend('get_chat_room_members_duration', true);
const errorRate = new Rate('error_rate');
const requestCount = new Counter('request_count');

export const options = {
    scenarios: {
        get_chat_room_members_steady: {
    ...
    thresholds: {
        'get_chat_room_members_duration': ['p(95)<3000'],
        'error_rate': ['rate<0.01'],
    },
};

const BASE_URL = 'http://localhost:8080';
// chat_room_id=1: 멤버 5명 (HOST 1 + MEMBER 4) → 3(고정) + 5(N) = 8번 쿼리
...
export function handleSummary(data) {
    const m = data.metrics.get_chat_room_members_duration.values;
